...
- **Vision Deployment:** AI training service (1 replica)
- **Vision Service:** ClusterIP service for internal communication
...
minikube service arc-nginx -n arc --url
...
kubectl scale deployment arc-frontend -n arc --replicas=0
kubectl scale deployment arc-nginx -n arc --replicas=0
kubectl scale deployment arc-vision -n arc --replicas=0

# Restart pods (scale deployment back to 1 replica)
kubectl scale deployment arc-backend -n arc --replicas=1
kubectl scale deployment arc-frontend -n arc --replicas=1
kubectl scale deployment arc-nginx -n arc --replicas=1
kubectl scale deployment arc-vision -n arc --replicas=1
